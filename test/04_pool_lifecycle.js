...
(async () => {
  ...
  inv(`register_asset --asset_id 1 --token ${USDC_SAC}`);
  ...
  async function doTransact(label, params, { send = true, expectFail = false } = {}) {
    const { witness, outputCommitment } = buildWitness({ ...params.build, auditor: { pubX: auditor.pubX, pubY: auditor.pubY }, assetId: 1n });
