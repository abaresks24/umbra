...
async function shieldArgs(auditor) {
  ...
  const note = new Note({ amount: 100n, assetId: 1n, owner: new Keypair() });
  const r = buildWitness({
    tree, inputs: [], outputs: [note], publicAmount: 100n, assetId: 1n,
...
(async () => {
  ...
  sh(`stellar contract invoke --id ${CID} --source shield --network testnet -- register_asset --asset_id 1 --token ${e.USDC_SAC}`);
